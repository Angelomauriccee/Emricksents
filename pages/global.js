...
fetch("/data/products.json")
  ...
  .then(products => {
    ...
    const brandName = pathArray[pathArray.length - 2]; // Extract brand name from URL

    console.log('Detected brand:', brandName); // Check if detection works

    // Filter products based on detected brand
    ...
    let filteredProducts = [...brandProducts]; // Track filtered products

    // Function to display products
    function displayFilteredProducts(productsToDisplay) {
      countDisplay.textContent = `Showing ${productsToDisplay.length} of ${brandProducts.length} products`;
    ...
    function applyFilters() {
      ...
      // ❌ FIXED: Used `brandProducts` instead of `amouroudProducts`
